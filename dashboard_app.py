...
def create_figure(df, title, func, figsize=(6, 3.5)): # حجم افتراضي أصغر
    if df.empty: 
        return None
    try:
        fig, ax = plt.subplots(figsize=figsize)
        func(fig, ax, df) 
        ax.set_title(title, fontsize=12, color='darkgreen', fontweight='bold', pad=10) # تصغير حجم عنوان الرسم
        plt.tight_layout()
...
def plot_top_brands(df_brands): 
    # استخدام الحجم الافتراضي المعدل، أو تحديد حجم مختلف إذا لزم الأمر
    return create_figure(df_brands, "Top 10 Sustainable Brands", figsize=(7, 4), # حجم أكبر قليلاً لهذا الرسم
        func=lambda fig, ax, df: (
            ...
            ax.set_xticklabels(df["brand_name"], rotation=rotation_angle, ha='right', fontsize=8), # تصغير حجم نص المحور X
            sns.despine(ax=ax)
        )
    )
...
def plot_material_status(df_material_sus): 
    ...
    fig, ax = plt.subplots(figsize=(4, 4)) # حجم أصغر لل Pie/Donut Chart
    ...
    wedges, texts = ax.pie(df_material_sus['sustainability_rating'], 
                           ...
                           textprops={'color': '#1B5E20', 'fontsize': 9, 'fontweight': 'bold'}) # تصغير حجم نص التسميات
    
    centre_circle = plt.Circle((0, 0), 0.70, fc='white')
    fig.gca().add_artist(centre_circle)

    ax.set_title('Avg. Rating by Material Status', fontsize=12, color='#1B5E20', fontweight='bold', pad=10) # تصغير حجم عنوان الرسم
    plt.tight_layout()
    return fig
...
def plot_eco_friendly_counts(eco_counts_series): 
    ...
    ax.pie(eco_counts_series.values, 
           ...
           startangle=90,
           textprops={'fontsize': 10}) # تصغير حجم نص النسبة المئوية
...
def plot_price_vs_sustainability(df_price_sus): 
    return create_figure(df_price_sus, "Price vs. Sustainability Rating", figsize=(7, 4), # حجم أكبر قليلاً
        func=lambda fig, ax, df: (
            ...
            ax.legend(title="Brand Category", title_fontsize=8, fontsize=7, bbox_to_anchor=(1.05, 1), loc="upper left") # تصغير حجم legend
        )
    )

# 11. Impact of Certification on Sustainability Rating
def plot_certification_impact(df_certification_avg): 
    return create_figure(df_certification_avg, "Impact of Certification on Rating", figsize=(7, 4), # حجم أكبر قليلاً
        func=lambda fig, ax, df: (
...
def main():
    ...
    with col_logo1:
        st.image("logo_ministry.png", width=100) # استبدلي "logo_ministry.png" باسم ملف شعار الوزارة
    with col_title:
        st.title("Sustainability Dashboard")
    with col_logo2:
        st.image("logo_project.png", width=100) # استبدلي "logo_project.png" باسم ملف شعار المشروع
    st.markdown("---")
